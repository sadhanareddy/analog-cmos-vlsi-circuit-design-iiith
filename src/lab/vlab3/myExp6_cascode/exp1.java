	...
	String Resistance_2 = "1000"; 
        ...
        public class node {
            ...
            public void update_end_points_mat(int img) {
		...
		else if (img == 2 || img == 9 ) // ground , VDD , INPUT , OUTPUT
                {
                    ...
                    for (int i = end_pointsX[0] - 4; i < end_pointsX[0] + 5; i++) {
                        for (int j = end_pointsY[0] - 4; j < end_pointsY[0] + 5; j++) {
                            if (img == 2) // ground
                            {
                                end_points_mat[i][j] = 12;
                            } else if (img == 9) //vdd
                            {
                                end_points_mat[i][j] = 13;
                            } 
                        }
                    }
                ...
                } else if (img ==11 || img==6) // AC Vol
                {
                    for (int k = 0; k < 2; k++) {
                        for (int i = end_pointsX[k] - 4; i < end_pointsX[k] + 5; i++) {
                            for (int j = end_pointsY[k] - 4; j < end_pointsY[k] + 5; j++) {
                                end_points_mat[i][j] = k + 16;
                            }
                        }
                    }
                }
...
URL tempfile = null;
            ...
            public void paint(Graphics g) {
              ...
              /*  g2d.setColor(Color.lightGray);
                ...
                for (i = 0; i < no_values - 1; i++) {
                    g2d.drawLine(40 + 2 * i, 400 - (int) Math.round(V_out[i] * 100000), 40 + 2 * (i + 1), 400 - (int) Math.round(V_out[i + 1] * 100000));
                }
...
System.out.println("Idhar paunch gaya");
		...
		for(i=0;i<10;i++)
		{
			g2d.drawString(strPre(.18*(i+1)),40+30*(i+1),450);
		}
		for(i=0;i<11;i++)
		{
		...
		for(i=0;i<15;i++)
		{
			g2d.drawString(strPre(.12*(i+1)),15,450-30*(i+1));
		}
		for(i=0;i<15;i++)
		{
			g2d.drawLine(35,450-30*(i+1),40,450-30*(i+1));
                ...
                g2d.drawString("I(vdc) ", 2, 200);
                g2d.drawString("(mA) ", 2, 213);
...
//Setting Top Panel ========================================================================== 
            ...
            JLabel heading = new JLabel("<html><FONT COLOR=WHITE SIZE=6 >Design and characterization of a cascode current-mirror </FONT></html>", JLabel.CENTER);
...
//			bottom.add(h );
        ...
        public boolean circuit_check() {
            int check_points_value[][] = new int[18][10]; // each index will store corresponding  component for points of (comp point no -> index )
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() == exp_list) {
                ...
                if (Nmos_l != null && Nmos_w != null && Capacitance != null && Resistance_1 != null && DC_vol != null  && circuit_check()) {
                    //		''' callJS();
                    URL php_file = null;
