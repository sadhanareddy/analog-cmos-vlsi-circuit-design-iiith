...
public class exp1_2 extends JApplet {
    ...
    public class MyPanel extends JPanel implements ActionListener, MouseMotionListener {
        ...
        String[] comp_str = { // This will store what should at the Dialog Box for each component
            ...
            "OUTPUT", "Current Src ", "Inductor ", // 10, 11 , 12
        ...
        public class node {
            ...
            public void make_end_points(int img) {
                ...
                } else if (img == 8 || img == 5 || img == 11) // Capacitor
                {
                    count_end_points = 2;
                    int a, b, c, d;
                    a = 0;
...
//				end_index++;
        ...
        public class myDialog extends JDialog implements ActionListener {
            ...
            public myDialog(JFrame fr, String comp, int node_no) {
                ...
                }/* else if (comp_node[node_no].img_no == 5) // dc voltage 
                {
                SpinnerModel voltage_model = new SpinnerNumberModel(1, //initial value
                0, //min
                1.8, //max
                0.2);  //step
                JLabel comp_name = new JLabel("<html><font size=4><b>" + comp + "</b></font></html>");//,icon[icon_no],JLabel.CENTER);
                ...
                voltage = new JSpinner(voltage_model);
                JLabel v = new JLabel("Select the Voltage :");
                JLabel v_unit = new JLabel("volts");
                
                del = new JButton("Delete Component");
                ok = new JButton("O.K");
                rotate = new JButton("Rotate");
                
                layout.putConstraint(SpringLayout.WEST, v, 20, SpringLayout.WEST, cp);
                layout.putConstraint(SpringLayout.NORTH, v, 60, SpringLayout.NORTH, cp);
                
                layout.putConstraint(SpringLayout.WEST, voltage, 20, SpringLayout.EAST, v);
                layout.putConstraint(SpringLayout.NORTH, voltage, 60, SpringLayout.NORTH, cp);
                
                layout.putConstraint(SpringLayout.WEST, v_unit, 10, SpringLayout.EAST, voltage);
                layout.putConstraint(SpringLayout.NORTH, v_unit, 60, SpringLayout.NORTH, cp);
                ...
                del.addActionListener(this);
                rotate.addActionListener(this);
                }*/ else if (comp_node[node_no].img_no == 11) // current 
                {
                    SpinnerModel voltage_model = new SpinnerNumberModel(100, //initial value
                            100, //min
                            1000, //max
                            100);  //step
                    JLabel comp_name = new JLabel("<html><font size=4><b>" + comp + "</b></font></html>");//,icon[icon_no],JLabel.CENTER);
                    ...
                    current = new JSpinner(voltage_model);
                    JLabel ct = new JLabel("Select the Current :");
                    JLabel ct_unit = new JLabel("micro_amperes");

                    del = new JButton("Delete Component");
                    ok = new JButton("O.K");
                    rotate = new JButton("Rotate");

                    layout.putConstraint(SpringLayout.WEST, ct, 20, SpringLayout.WEST, cp);
                    layout.putConstraint(SpringLayout.NORTH, ct, 60, SpringLayout.NORTH, cp);

                    layout.putConstraint(SpringLayout.WEST, current, 20, SpringLayout.EAST, ct);
                    layout.putConstraint(SpringLayout.NORTH, current, 60, SpringLayout.NORTH, cp);

                    layout.putConstraint(SpringLayout.WEST, ct_unit, 10, SpringLayout.EAST, current);
                    layout.putConstraint(SpringLayout.NORTH, ct_unit, 60, SpringLayout.NORTH, cp);
                    ...
                    del.addActionListener(this);
                    rotate.addActionListener(this);
                } else if (comp_node[node_no].img_no == 2 || comp_node[node_no].img_no == 9 || comp_node[node_no].img_no == 1 || comp_node[node_no].img_no == 7 || comp_node[node_no].img_no == 5) // terminal
                {
                    JLabel comp_name = new JLabel("<html><font size=4><b>" + comp + "</b></font></html>");//,icon[icon_no],JLabel.CENTER);
            ...
            public void actionPerformed(ActionEvent e) {
                if (e.getSource() == ok) {
                    ...
                    } else if (comp_node[node_index].img_no == 5) //Voltage
                    {
                        Voltage = get_voltage();
            ...
            public void mouseClicked(MouseEvent me) {
                ...
                if (img_button_pressed == -1) // for selecting anything on work panel
                {
                    ...
                    } else if (work_mat[work_x][work_y] != -1) // there is a comp on (work_x , work_y) here temp rep inder of comp_nodearray
                    {
                        ...
                        if (temp1 == 1 || temp1 == 7 || temp1 == 8 || temp1 == 2 || temp1 == 9 || temp1 == 5 || temp1 == 11) {
                ...
                } else // For adding comp 
                {
                    ...
                    } else if (img_button_pressed == 8 || img_button_pressed == 5 || img_button_pressed == 11) // capicitor  
                    {
...
////////			int no_values = 100 ;
            ...
            public void make_graph(double Len, double Wid, double Cur) {
                int incr = 0;
                double W = 240;//240.0;
                double L = 180;//180.0;
                double MuCox = .0001;
                ...
                for (incr = 0; incr < 100; incr++) {
                    in_I[incr] = (Id + incr * .001) * 1000;
                    data[incr] = Math.sqrt(2 * MuCox * (W / L) * in_I[incr]) * 1000;
                }
//				fileToRead ="outfile";
//				URL url = null;
//				try
...
//					}
            ...
            public void paint(Graphics g) {
                ...
                g2d.setColor(Color.red);
                g2d.drawString("Gm", 0, 250);
		g2d.drawString("(mA/V)",0,260);
                g2d.drawString("Id (microAmperes)--->", 250, 480);

                g2d.setColor(Color.black);
                g2d.setStroke(new BasicStroke(1));
                int iii, jjj;
                for (iii = 20; iii < 350; iii += 20) {
                    for (jjj = 40; jjj < 470; jjj += 5) {
                        g2d.fillOval(iii, jjj, 2, 2);
                    }
                }
                for (iii = 20; iii < 350; iii += 5) {
                    for (jjj = 40; jjj < 470; jjj += 20) {
                        g2d.fillOval(iii, jjj, 2, 2);
                    }
                }
		...
		{
			//g2d.drawString(strPre((in_I[iii])),40+30*(iii+1),450);
		}
		for(iii=0;iii<10;iii++)
		{
			//g2d.drawLine(40+30*(iii+1),440,40+30*(iii+1),435);
		}
		for(iii=0;iii<15;iii++)
		{
			//g2d.drawString(strPre(data[iii]),25,450-30*(iii+1));
		}
		for(iii=0;iii<15;iii++)
		{
			//g2d.drawLine(35,450-30*(iii+1),40,450-30*(iii+1));
		}
                g2.setPaint(Color.red);
                for (int j = 0; j < data.length; j++) {
                    int x = x0 + (int) (xScale * in_I[j]);
...
//			JLabel wave_head = new JLabel("This is the wave simulation output");
	    ...
	    //JPanel notes = new JPanel();
            //JLabel note_head = new JLabel("<html><FONT COLOR=black SIZE=5>NOTES</FONT></html>", JLabel.CENTER);
            ...
            //notesSplitPane.setDividerLocation(0.2);
           ...
           // Dimension dim = new Dimension(400, 250);
            ...
            //jTextField1.setFont(new Font("Papyrus",Font.PLAIN,14));//
                    //"Monospaced", Font.PLAIN,14));
            //jTextField1.setWrapStyleWord(true);
           //jTextField1.setText("This exp plots the variation in the transconductance with drain current.\n\nThe transconductance is proportional to the square root of drain current when aspect ratio is assumed constant.\n\nThe transconductance is proportional to drain current, when overdrive is assumed constant."); 
            //jTextField1.setEditable(false);
         //   jTextField1.setAlignmentY(BOTTOM_ALIGNMENT);
...
//Setting Top Panel ========================================================================== 
            ...
            JLabel heading = new JLabel("<html><FONT COLOR=WHITE SIZE=10 ><B>Variation of Transconductance (Gm) with Drain Current (Id)</B></FONT></html>", JLabel.CENTER);
...
//			bottom.add(h );
        ...
        public boolean circuit_check() {
            ...
            if (exp_type == 0) // Complementary 
            {
                ...
                while (check_points_value[0][l] != -1) {
                    if (check_points_value[0][l] != 5 && check_points_value[0][l] != 6) {
                        return false;
                    }
                    l++;
                }
                ...
                while (check_points_value[4][l] != -1) {
                    if (check_points_value[4][l] != 7 && check_points_value[4][l] != 2) {
                        return false;
                    }
                    l++;
                }
                ...
                while (check_points_value[3][l] != -1) {
                    if (check_points_value[3][l] != 7 && check_points_value[3][l] != 2) {
                        return false;
                    }
                    l++;
                }
                ...
                while (check_points_value[5][l] != -1) {
                    if (check_points_value[5][l] != 0 && check_points_value[5][l] != 8) {
                        return false;
                    }
                    l++;
                }
                ...
                while (check_points_value[6][l] != -1) {
                    if (check_points_value[6][l] != 8 && check_points_value[6][l] != 0) {
                        return false;
                    }
                    l++;
                }
                ...
                while (check_points_value[2][l] != -1) {
                    if (check_points_value[2][l] != 3 && check_points_value[2][l] != 4) {
                        return false;
                    }
                    l++;
                }
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() == exp_list) {
                ...
                if (true){//((Pmos_l != null && Pmos_w != null) || (Nmos_l != null && Nmos_w != null)) && Voltage != null && Current != null && circuit_check()) {
                    //		callJS();
                    URL php_file = null;
                    URLConnection c = null;
                    String encoded = "comp=" + URLEncoder.encode(Pmos_l + "_" + Pmos_w + "_" + Nmos_l + "_" + Nmos_w);
                    URL CGIurl = null;
                    try {
                        if (exp_type == 0) {
                    ...
                    waveRightPanel.make_graph(Double.parseDouble(Pmos_l), Double.parseDouble(Pmos_w), Double.parseDouble(Current));// Read file OUTFILE and draw the 
