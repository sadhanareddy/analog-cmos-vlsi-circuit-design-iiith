...
public class exp1_2_2 extends JApplet {
        ...
        public class myDialog extends JDialog implements ActionListener {
            ...
            public myDialog(JFrame fr, String comp, int node_no) {
                ...
                } else if (comp_node[node_no].img_no == 11) // current 
                {
                    ...
                    del.addActionListener(this);
                    rotate.addActionListener(this);
                } */else if (comp_node[node_no].img_no == 2 || comp_node[node_no].img_no == 9 || comp_node[node_no].img_no == 11 || comp_node[node_no].img_no == 5) // terminal
                {
                    JLabel comp_name = new JLabel("<html><font size=4><b>" + comp + "</b></font></html>");//,icon[icon_no],JLabel.CENTER);
            ...
            String get_width() {
                return width.getValue().toString();
            }
...
////////			int no_values = 100 ;
            ...
            public void make_graph(double Len, double Wid, double Cur) {
                int incr = 0;
                double W = Wid;//240.0;
		System.out.println("I got this "+W);
                double L = 180.0;//180.0;
                double MuCox = .0001;
                double Id = .0001;
                double Vgs=1.0;
                for (incr = 0; incr < 100; incr++) {
                    in_W[incr] = (W*10 + incr * 100) * .01;
                    data[incr] = MuCox*(in_W[incr]/L)*(Vgs-0.5) * 100000;
                }
//				fileToRead ="outfile";
//				URL url = null;
//				try
...
//					}
		...
		g2d.drawString("(mA/V)",0,260);
                g2d.drawString("Width (nm)--->", 275, 490);
                
                g2d.setColor(Color.black);
                g2d.setStroke(new BasicStroke(1));
                int iii, jjj;
                for (iii = 20; iii < 380; iii += 20) {
                    for (jjj = 40; jjj < 470; jjj += 5) {
                        g2d.fillOval(iii, jjj, 2, 2);
                    }
                }
                for (iii = 20; iii < 380; iii += 5) {
...
//			JLabel wave_head = new JLabel("This is the wave simulation output");
	    ...
	    //JPanel notes = new JPanel();
            ...
            //jTextField1.setText("The variation in the transconductance with aspect ratio is plotted in this graph.\n\nThe transconductance is proportional to aspect ration when overdrive voltage is constant.\n\nThe transconductance is directly proportional to the square root of aspect ratio when drain current is constant.");
...
//Setting Top Panel ========================================================================== 
            ...
            JLabel heading = new JLabel("<html><FONT COLOR=WHITE SIZE=10 ><B>Variation of Transconductance (Gm) with Aspect Ratio (W/L)</B></FONT></html>", JLabel.CENTER);
