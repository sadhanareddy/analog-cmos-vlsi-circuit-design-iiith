            ...
            public void update_end_points_mat(int img) {
                ...
                } else if (img == 5) // DC Vol 
                {
                    for (int k = 0; k < 2; k++) {
                        for (int i = end_pointsX[k] - 4; i < end_pointsX[k] + 5; i++) {
                            for (int j = end_pointsY[k] - 4; j < end_pointsY[k] + 5; j++) {
                                end_points_mat[i][j] = k + 11-3;
                            }
                        }
                    }
                } else if (img ==11) // AC Vol
                {
                    for (int k = 0; k < 2; k++) {
                        for (int i = end_pointsX[k] - 4; i < end_pointsX[k] + 5; i++) {
                            for (int j = end_pointsY[k] - 4; j < end_pointsY[k] + 5; j++) {
                                end_points_mat[i][j] = k + 13-3;
                            }
                        }
                    }
                }
...
System.out.println("Idhar paunch gaya");
		...
		for(i=0;i<15;i++)
		{
			g2d.drawString(strPre(.05*(i+1)),15,450-30*(i+1));
		}
		for(i=0;i<15;i++)
		{
...
//Setting Top Panel ========================================================================== 
            ...
            JLabel heading = new JLabel("<html><FONT COLOR=WHITE SIZE=6 > Design and characterization of a basic current-mirror</FONT></html>", JLabel.CENTER);
...
//			bottom.add(h );
        ...
        public boolean circuit_check() {
            int check_points_value[][] = new int[12][10]; // each index will store corresponding  component for points of (comp point no -> index )
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() == exp_list) {
                ...
                if (Nmos_l != null && Nmos_w != null &&  circuit_check()) {
                    //	'''	callJS();
                    URL php_file = null;
